...
module.exports = {
  ...
  async execute(client) {
    console.log(`[API] ${client.user.username} está ON en ${client.guilds.cache.size} servidores`);
    ...
    client.manager.on("nodeConnect", async node => {
      console.log(`[NODE] "${node.options.identifier}" conectado correctamente.`);
    ...
    client.manager.on("trackStart", (player, track) => {
      ...
      let np = new MessageEmbed()
        .setColor("")
        .setAuthor(` 🎵 REPRODUCIENDO 🎵`, `${track.requester.displayAvatarURL({ dynamic: true })}`)
        .setDescription(` 
