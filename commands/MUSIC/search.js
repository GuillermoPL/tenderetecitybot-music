...
run:async (client, message, args) => {
  ...
  if (!args.length) {
    let embed = new MessageEmbed()
      .setColor("RED")
      .setDescription(` ¡Debes introducir el nombre de una canción o su enlace!`)
  ...
  if (!play) {
    ...
    if (!channel.joinable) {
      let embed = new MessageEmbed()
        ...
        .setDescription(" No tengo permisos suficientes como para unirme a tu canal.")
      return message.channel.send(embed)
    }
  ...
  try {
    ...
    let embed = new MessageEmbed()
      .setColor("RED")
      .setDescription( ` No puedo encontrar ningún resultado para: \`${search}\``)
    return message.channel.send(embed)
  }
  ...
  switch (res.loadType) {
    case 'SEARCH_RESULT':
      ...
      let embed = new MessageEmbed()
        .setColor(color)
        .setTitle("¡SELECCIONA TU CANCIÓN!")
