...
run: async (client, message, args) => {
 ...
 let player = client.manager.players.get(message.guild.id);
		...
		if (!message.member.voice.channel.permissionsFor(message.guild.me).has('CONNECT')) {
			return embed("Necesito **permisos de conectar** en tu canal de voz", message.channel).then(m => m.delete({ timeout: 10000 }));
		}
		...
		if (!message.member.voice.channel.permissionsFor(message.guild.me).has('SPEAK')) {
			return embed("Necesito **permisos de hablar** en tu canal de voz",message.channel).then(m => m.delete({ timeout: 10000 }));
		}
		...
		embed("Me he unido correctamente a tu canal de voz.",message.channel)
			...
			try {
				...
				const embed = new MessageEmbed()
					.setColor("GREEN")
					.setDescription("Ya estoy en tu canal de voz.");
