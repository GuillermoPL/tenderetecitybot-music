...
module.exports = {
  ...
  if (!player) {
    let embed = new MessageEmbed()
      .setColor("RED")
      .setDescription(" Hay **0** canciones en la cola")
  ...
  if (songs === null) {
    songsForPageContent = `PARA AÑADIR UNA CANCIÓN USA:!PLAY O !SEARCH`
    return message.channel.send(` \`REPRODUCIENDO: ${player.queue.current.title}\` \`\`\`fix\n${songsForPageContent}\`\`\` `)
  }
  ...
  const msg = await message.channel.send(`\` **REPRODUCIENDO**: ${player.queue.current.title}\` \`\`\`fix\n${songsForPageContent.join('\n')}\`\`\` `)
  await msg.react('⬆️')
  await msg.react('⬇️')
  ...
  function forward() {
    ...
    msg.edit(`\`REPRODUCIENDO: ${player.queue.current.title}\` \`\`\`fix\n${songsForPageContent.join('\n')}\`\`\` `)
  }
